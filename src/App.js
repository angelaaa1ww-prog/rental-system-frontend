...
function injectStyles(T) {
  ...
  el.textContent = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .btn-sms:hover:not(:disabled){transform:translateY(-1px);}
    ...
    .btn-theme:hover{border-color:${T.accent};}
    ...
    /* ── SIDEBAR MOBILE ── */
    .sidebar-wrap{
      width:250px;background:${T.sidebar};
    ...
    @media(min-width:769px){
      .sidebar-wrap{position:relative;transform:none!important;}
    }
  `;
}
    ...
    <div style={{position:"fixed",top:20,right:20,zIndex:9999,display:"flex",flexDirection:"column",gap:8}}>
      {toasts.map(t => (
        <div key={t.id} style={{
          ...
          background:t.type==="success"?"linear-gradient(135deg,#0A7A4B,#1DB87A)":
                     t.type==="error"  ?"linear-gradient(135deg,#A32D2D,#E24B4A)":
                                        "linear-gradient(135deg,#1459A0,#2D7DD2)",
...
function Av({name, size=30}) {
  return (
    <div style={{width:size,height:size,minWidth:size,background:"linear-gradient(135deg,#0A7A4B,#1DB87A)",borderRadius:"50%",display:"inline-flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:size*0.42,color:"white",marginRight:8}}>
      {name?.[0]?.toUpperCase()||"?"}
    </div>
  );
}
...
export default function App() {
  ...
  // ✅ inject styles — eslint disable needed here
  ...
  /* ── house form — ✅ rent is now manual (no auto-fill) ── */
  ...
  // ✅ handle resize — sidebar open on desktop, closed on mobile
  useEffect(() => {
  ...
  const safeFetch = async (url, opts = {}) => {
    try {
      ...
      show("Connection failed — server may be starting up. Try again in 30s.","error");
      return null;
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setLoginLoading(false);
      show("⏳ Server waking up (free tier). Wait ~30 seconds and try again.","error");
    }
  };
  ...
  const addHouse = async () => {
    ...
    if (res) {
      ...
      setHouseNumber(''); setLocation(''); setRent(''); setBedrooms(1); setApartment('A');
      loadAll();
    }
  };
        ...
        <div className={`sidebar-wrap${sidebarOpen?" open":""}`}>
          ...
          <div style={{padding:"22px 18px 14px",borderBottom:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:44,height:44,background:"linear-gradient(135deg,#0A7A4B,#1DB87A)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>🏢</div>
              <div>
            ...
            <div style={{padding:"16px",maxWidth:1100,margin:"0 auto"}}>
                    ...
                    <>
                      ...
                      <div style={C()}>
                        ...
                        <div style={{background:T.cardBorder,borderRadius:999,height:12,overflow:"hidden"}}>
                          <div style={{width:`${dashData.occupancyRate}%`,height:"100%",borderRadius:999,transition:"width 1s ease",
                            background:dashData.occupancyRate>=80?"linear-gradient(90deg,#0A7A4B,#1DB87A)":dashData.occupancyRate>=50?"linear-gradient(90deg,#B8800A,#F0C030)":"linear-gradient(90deg,#8B1A1A,#E24B4A)"}}/>
                        </div>
                ...
                <div style={{animation:"fadeUp 0.35s ease"}}>
                  <div style={C()}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}}>
                      <Field lbl="House Number">
                        <input className="inp" placeholder="e.g. A101" value={houseNumber} onChange={e => setHouseNumber(e.target.value)}/>
                      </Field>
                      <Field lbl="Location">
                        <input className="inp" placeholder="e.g. Kiambu Road" value={location} onChange={e => setLocation(e.target.value)}/>
                      </Field>
                      <Field lbl="Apartment">
                      ...
                      {/* ✅ FIXED: manual rent input */}
                      <Field lbl="Rent (KES)">
                        <input className="inp" type="number" placeholder="Enter rent e.g. 8500"
                  ...
                  {!filteredHouses.length && <Empty T={T} icon="🏠" text="No houses found" sub="Add a new house using the form above"/>}
                </div>
              )}
              ...
              {page==="tenants" && (
                <div style={{animation:"fadeUp 0.35s ease"}}>
                  ...
                  <p style={{fontSize:12,color:T.subtext,fontWeight:600,marginBottom:12}}>
                        ...
                        {filteredTenants.map(t => {
                          ...
                          return (
                            <div key={t._id} style={{background:T.card,borderRadius:14,border:`1px solid ${T.cardBorder}`,padding:"16px"}}>
                              ...
                              {/* Assign house */}
                              <select className="inp" style={{marginBottom:12}} defaultValue=""
                                onChange={e => assignHouse(t._id, e.target.value)}>
                              ...
                              <div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>
                                <input className="inp" placeholder="Custom SMS (optional)"
                  ...
                  {!dataLoading && !filteredTenants.length && <Empty T={T} icon="👤" text="No tenants found" sub="Add a new tenant using the form above"/>}
                </div>
              )}
                      ...
                      <div style={{marginBottom:16}}>
                        <Tag bg={T.accentLight} color={T.accent}>
                          {houses.find(h => String(h._id)===String(profileTenant.house))
                            ? `🏠 House ${houses.find(h => String(h._id)===String(profileTenant.house))?.houseNumber}`
                            : "No House Assigned"}
                        </Tag>
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:8}}>
