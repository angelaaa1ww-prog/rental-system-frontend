...
/* ══════ ICONS ══════ */
const ICONS = {
  building: '<path d="M4 22V4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v18" /><path d="M14 22v-4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v4" /><path d="M10 6h4" /><path d="M10 10h4" /><path d="M10 14h4" />',
  ...
  banknote: '<rect x="2" y="6" width="20" height="12" rx="2" ry="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/>',
  idCard: '<rect x="2" y="5" width="20" height="14" rx="2" ry="2"/><path d="M16 10h2"/><path d="M16 14h2"/><circle cx="9" cy="10" r="2"/><path d="M6 15v-1a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v1"/>',
  eye: '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>',
  phone: '<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>',
  megaphone: '<polygon points="11 19 11 23 15 23 15 19"/><path d="M22 12A10 10 0 0 1 12 22"/><path d="M12 2A10 10 0 0 1 22 12"/><path d="M15 12A3 3 0 0 1 12 15"/><path d="M12 9A3 3 0 0 1 15 12"/><polygon points="3 11 3 13 8 15 8 9"/>',
    ...
    <div style={{position:"fixed",top:16,right:16,zIndex:9999,display:"flex",flexDirection:"column",gap:8,maxWidth:300}}>
      {list.map(t => (
        <div key={t.id} style={{
          ...
          <span>{t.type==="success"?<Icon name="checkCircle" size={16}/>:t.type==="error"?<Icon name="xCircle" size={16}/>:<Icon name="info" size={16}/>}</span>{t.msg}
        </div>
      ))}
    </div>
  );
    ...
    <div className="modal-bg" onClick={e => e.target===e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",color:T.subtext,lineHeight:1,display:"flex"}}><Icon name="x" size={24}/></button>
        </div>
        {children}
      </div>
    </div>
    ...
    return (
      <div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
          <div style={{width:40,height:40,background:"linear-gradient(135deg,#059669,#10B981)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}><Icon name="checkCircle" size={20}/></div>
          <div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
          ...
          <button className="btn-red" onClick={disable2FA} disabled={loading} style={{padding:"11px 16px"}}><Icon name="unlock" size={20}/> Disable 2FA</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {!qrCode ? (
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{width:40,height:40,background:"linear-gradient(135deg,#DC2626,#EF4444)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}><Icon name="unlock" size={20}/></div>
            <div>
          ...
          <button className="btn-green" onClick={setup2FA} disabled={loading}>{loading?"Setting up...":<><Icon name="shield" size={20}/> Setup 2FA</>}</button>
        </div>
      ) : (
        <div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
            ...
            <button className="btn-green" onClick={enable2FA} disabled={loading} style={{padding:"11px 16px"}}>{loading?"Verifying...":<><Icon name="checkCircle" size={20}/> Enable 2FA</>}</button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google) {
        window.google.accounts.id.initialize({ client_id: process.env.REACT_APP_GOOGLE_CLIENT_ID || '', callback: handleGoogleLogin }); window.google.accounts.id.renderButton(document.getElementById('google-btn'), { theme: 'outline', size: 'large', width: '100%' });
      }
    };
  ...
  if(!loggedIn) return (
    <>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:dark?"linear-gradient(135deg,#020617,#0B1120)":"linear-gradient(135deg,#1E1B4B,#312E81)",position:"relative",overflow:"hidden",padding:16}}>
        ...
        <button className="btn-theme" onClick={()=>setDark(d=>!d)} style={{position:"fixed",top:16,right:16,zIndex:10}}>{dark?<Icon name="sun" size={20}/>:<Icon name="moon" size={20}/>}</button>
        ...
        <div style={{background:dark?"rgba(15,23,42,0.95)":"rgba(255,255,255,0.95)",borderRadius:22,padding:"36px 28px",width:"min(400px,100%)",boxShadow:"0 20px 60px rgba(0,0,0,0.4)",position:"relative",zIndex:1,border:`1px solid ${T.cardBorder}`}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            <div style={{width:64,height:64,background:"linear-gradient(135deg,#4F46E5,#818CF8)",borderRadius:18,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",fontSize:30}}><Icon name="building" size={24}/></div>
          ...
          {welcome ? (
            <div style={{textAlign:"center",padding:"16px 0"}}>
              <div style={{fontSize:46,marginBottom:10}}><Icon name="checkCircle" size={40}/></div>
              ...
              {requires2FA ? (
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  <div style={{textAlign:"center",marginBottom:6}}>
                    <div style={{fontSize:40,marginBottom:8}}><Icon name="shield" size={20}/></div>
  ...
  ════════════════════════════════════ */
  const navItems = [
    {id:"dashboard",label:"Dashboard",  icon: <Icon name="barChart" size={20}/>},
    {id:"houses",   label:"Houses",     icon: <Icon name="home" size={20}/>},
    {id:"tenants",  label:"Tenants",    icon: <Icon name="user" size={20}/>},
    {id:"payments", label:"Payments",   icon: <Icon name="creditCard" size={20}/>},
    {id:"reports",  label:"Reports",    icon: <Icon name="fileText" size={20}/>},
    {id:"sms",      label:"SMS Alerts", icon: <Icon name="messageSquare" size={20}/>},
  ...
  return (
    <>
      ...
      {/* EDIT TENANT MODAL */}
      {editT && (
        <Modal title={<><Icon name="edit" size={14}/> Edit Tenant</>} onClose={()=>setEditT(null)} T={T}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10,marginTop:6}}>
              <button className="btn-green" onClick={saveTenant} style={{flex:1}}><Icon name="save" size={14}/> Save</button>
      ...
      {/* EDIT HOUSE MODAL */}
      {editH && (
        <Modal title={<><Icon name="edit" size={14}/> Edit House</>} onClose={()=>setEditH(null)} T={T}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10,marginTop:6}}>
              <button className="btn-green" onClick={saveHouse} style={{flex:1}}><Icon name="save" size={14}/> Save</button>
      ...
      {/* DELETE MODAL */}
      {delConf && (
        <Modal title={<><Icon name="alertTriangle" size={20}/> Confirm Delete</>} onClose={()=>setDelConf(null)} T={T}>
          ...
          <div style={{display:"flex",gap:10}}>
            <button className="btn-red" onClick={doDelete} style={{flex:1,padding:"11px 0",fontSize:14}}><Icon name="trash" size={14}/> Delete</button>
      ...
      <div style={{display:"flex",height:"100dvh",minHeight:"-webkit-fill-available",overflow:"hidden",position:"relative"}}>
        ...
        <div className={`sidebar${sidebar?" open":""}`}
          ...
          <div style={{padding:"20px 16px 14px",borderBottom:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:42,height:42,background:"linear-gradient(135deg,#0A7A4B,#1DB87A)",borderRadius:11,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}><Icon name="building" size={24}/></div>
              <div>
          ...
          <div style={{padding:"10px 8px 18px",flexShrink:0}}>
            <div className="nav-item logout" onClick={logout}>
              <span style={{fontSize:16}}><Icon name="logOut" size={20}/></span> Logout
            </div>
          </div>
        </div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:T.bg,minWidth:0}}>
          ...
          <div className="no-print" style={{background:T.topbar,borderBottom:`1px solid ${T.topbarBorder}`,padding:"11px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,zIndex:50}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <button onClick={()=>setSidebar(o=>!o)} style={{background:"none",border:"none",cursor:"pointer",color:T.subtext,lineHeight:1,display:"flex"}}><Icon name="menu" size={24}/></button>
              <div>
                <h1 style={{fontSize:15,fontWeight:700,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"42vw"}}>
                  {page==="profile"&&profTenant?`<Icon name="user" size={20}/> ${profTenant.name}`:cur?`${cur.icon} ${cur.label}`:""}
                </h1>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              {reminders.length>0&&<div style={{background:"#FCEBEB",color:"#A32D2D",fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:999}}><Icon name="alertTriangle" size={20}/>{reminders.length}</div>}
              <button className="btn-outline" onClick={loadAll} style={{padding:"6px 10px",fontSize:12}}><Icon name="check" size={14} style={{opacity:0}}/><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.59-9.21l5.67-5.67"/></svg></button>
              <button className="btn-theme" onClick={()=>setDark(d=>!d)}>{dark?<Icon name="sun" size={20}/>:<Icon name="moon" size={20}/>}</button>
          ...
          <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
            <div style={{padding:"14px",maxWidth:1080,margin:"0 auto"}}>
                ...
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  {loading?(
                    ...
                    <>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:16}}>
                        {[
                          {l:"Total Houses", v:dash.totalHouses, bg:"linear-gradient(135deg,#0A3D2B,#0A7A4B)", icon: <Icon name="home" size={20}/>},
                          {l:"Occupied",     v:dash.occupied,    bg:"linear-gradient(135deg,#1459A0,#2D7DD2)", icon: <Icon name="checkCircle" size={20}/>},
                          {l:"Vacant",       v:dash.available,   bg:"linear-gradient(135deg,#7B4F00,#C8960A)", icon: <Icon name="key" size={20}/>},
                          {l:"Income",       v:`KES ${(dash.totalIncome||0).toLocaleString()}`, bg:"linear-gradient(135deg,#1A4A1A,#2E8B2E)", icon: <Icon name="dollarSign" size={20}/>, sm:true},
                          {l:"Overdue",      v:dash.overdueCount,bg:dash.overdueCount>0?"linear-gradient(135deg,#6B1A1A,#D63B3B)":"linear-gradient(135deg,#0A3D2B,#0A7A4B)", icon:dash.overdueCount>0?<Icon name="alertTriangle" size={20}/>:<Icon name="check" size={20}/>},
                          {l:"Tenants",      v:tenants.length,   bg:"linear-gradient(135deg,#1A1A6B,#534AB7)", icon: <Icon name="user" size={20}/>},
                        ].map(c=>(
                      ...
                      <div style={card()}>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:12,color:T.subtext}}>
                          <span><span style={{width:10,height:10,borderRadius:"50%",background:"#1DB87A",display:"inline-block",marginRight:4}}></span> {dash.occupied} occupied</span><span><Icon name="key" size={20}/> {dash.available} vacant</span>
                        </div>
                      </div>
                      ...
                      <div style={card()}>
                        ...
                        {!dash.overdueTenants?.length?<Empty T={T} icon={<Icon name="checkCircle" size={40}/>} text="All tenants are up to date!"/>:(
              ...
              {page==="houses"&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    <h2 style={cTit}><Icon name="home" size={20}/> Add New House</h2>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:12}}>
                      ...
                      {/* <Icon name="checkCircle" size={20}/> Manual rent input */}
                  ...
                  <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
                    <div style={{flex:1,minWidth:160,position:"relative"}}>
                      <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:T.subtext,pointerEvents:"none"}}><Icon name="search" size={16}/></span>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      {["all","occupied","vacant"].map(f=>(
                        <button key={f} onClick={()=>setHFilter(f)} style={{padding:"9px 12px",borderRadius:9,border:`1.5px solid ${hFilter===f?T.accent:T.cardBorder}`,background:hFilter===f?T.accent:"transparent",color:hFilter===f?"white":T.subtext,fontWeight:600,fontSize:12,cursor:"pointer",transition:"all 0.2s"}}>
                          {f==="all"?"All":f==="occupied"?<><Icon name="lock" size={20}/> Occ</>:<><Icon name="key" size={20}/> Vac</>}
                        </button>
                      ))}
                    </div>
                  </div>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          {apH.map(h=>(
                            <div key={h._id} style={{background:T.card,borderRadius:12,border:`1px solid ${T.cardBorder}`,padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                <div style={{width:38,height:38,background:h.status==="occupied"?"linear-gradient(135deg,#6B1A1A,#D63B3B)":"linear-gradient(135deg,#0A3D2B,#0A7A4B)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17}}>{h.status==="occupied"?<Icon name="lock" size={20}/>:<Icon name="key" size={20}/>}</div>
                                <div>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                                ...
                                <button className="btn-ghost" onClick={()=>setEditH({...h})}><Icon name="edit" size={14}/></button>
                                <button className="btn-red" onClick={()=>setDelConf({type:"house",id:h._id,name:h.houseNumber})}><Icon name="trash" size={14}/></button>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  {!fHouses.length&&<Empty T={T} icon={<Icon name="home" size={20}/>} text="No houses found" sub="Add a house above"/>}
                </div>
              )}
              ...
              {page==="tenants"&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    {/* <Icon name="checkCircle" size={20}/> Simple form: name + phone only */}
                    <h2 style={cTit}><Icon name="user" size={20}/> Add New Tenant</h2>
                        ...
                        return (
                          <div key={t._id} style={{background:T.card,borderRadius:14,border:`1px solid ${T.cardBorder}`,padding:"16px"}}>
                            ...
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10,marginBottom:12}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                ...
                                <div>
                                  ...
                                  {t.idNumber&&<p style={{fontSize:11,color:T.subtext}}><Icon name="idCard" size={14}/> ID: {t.idNumber}</p>}
                                </div>
                              </div>
                              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                <button className="btn-outline" style={{padding:"6px 10px",fontSize:11}} onClick={()=>openProfile(t)}><Icon name="eye" size={14}/> Profile</button>
                                <button className="btn-ghost" onClick={()=>setEditT({...t})}><Icon name="edit" size={14}/></button>
                                <button className="btn-red" onClick={()=>setDelConf({type:"tenant",id:t._id,name:t.name})}><Icon name="trash" size={14}/></button>
                              </div>
                            </div>
                            ...
                            <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
                              ...
                              <button className="btn-blue" onClick={()=>cashPay(t._id)}><Icon name="banknote" size={14}/> Record</button>
                            </div>
                            ...
                            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                              ...
                              <button className="btn-orange" onClick={()=>sendSMS(t._id,t.phone)} disabled={smsBusy[t._id]}>
                                {smsBusy[t._id]?"...":<><Icon name="messageSquare" size={20}/> SMS</>}
                              </button>
                            </div>
                          </div>
                        );
                  ...
                  {!loading&&!fTenants.length&&<Empty T={T} icon={<Icon name="user" size={20}/>} text="No tenants found" sub="Add a tenant above"/>}
                </div>
              )}
              ...
              {page==="profile"&&profTenant&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <button className="btn-back" onClick={()=>nav(fromPage)} style={{marginBottom:16}}>
                    <Icon name="chevronLeft" size={16}/> Back to {fromPage.charAt(0).toUpperCase()+fromPage.slice(1)}
                  </button>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:14}}>
                    ...
                    <div style={card({textAlign:"center"})}>
                      ...
                      <p style={{fontSize:13,color:T.subtext,marginBottom:14}}><Icon name="phone" size={14}/> {profTenant.phone}</p>
                      <div style={{marginBottom:14}}>
                        <Tag bg={T.accentLight} color={T.accent}>
                          {houses.find(h=>String(h._id)===String(profTenant.house))
                            ?`<Icon name="home" size={20}/> House ${houses.find(h=>String(h._id)===String(profTenant.house))?.houseNumber}`
                            :"No House Assigned"}
                        </Tag>
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:8}}>
                        <button className="btn-ghost" onClick={()=>setEditT({...profTenant})}><Icon name="edit" size={14}/> Edit Details</button>
                        <button className="btn-orange" onClick={()=>sendSMS(profTenant._id,profTenant.phone)}><Icon name="messageSquare" size={20}/> Send SMS</button>
                        <button className="btn-red" onClick={()=>setDelConf({type:"tenant",id:profTenant._id,name:profTenant.name})}><Icon name="trash" size={14}/> Delete</button>
                      </div>
                    </div>
                    ...
                    <div style={card()}>
                      ...
                      {!profPays.length?<Empty T={T} icon={<Icon name="creditCard" size={20}/>} text="No payments yet"/>:(
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                ...
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}}>
                      <h2 style={{...cTit,margin:0}}><Icon name="creditCard" size={20}/> Payment History</h2>
                    ...
                    {!payments.length?<Empty T={T} icon={<Icon name="creditCard" size={20}/>} text="No payments yet"/>:(
                      <div style={{overflowX:"auto"}}>
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                          ...
                          <tbody>
                            {payments.map((p,i)=>(
                              <tr key={p._id} style={{background:i%2===0?T.rowAlt:T.card}}>
                                ...
                                <td style={TD}><Tag bg={p.status==="confirmed"?"#E1F5EE":p.status==="pending"?"#FAEEDA":"#FCEBEB"} color={p.status==="confirmed"?"#0F6E56":p.status==="pending"?"#854F0B":"#A32D2D"}>{p.status||"confirmed"}</Tag></td>
                                <td style={TD}><span style={{fontSize:11,color:T.subtext}}>{p.createdAt?new Date(p.createdAt).toLocaleDateString("en-KE",{day:"numeric",month:"short",year:"numeric"}):"—"}</span></td>
                                <td style={TD}><button className="btn-red" style={{padding:"3px 8px",fontSize:10}} onClick={()=>setDelConf({type:"payment",id:p._id,name:`KES ${p.amount} payment`})}><Icon name="trash" size={14}/></button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
              ...
              {page==="reports"&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    <h2 style={cTit}><Icon name="fileText" size={20}/> Monthly Report</h2>
                    <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
                      ...
                      {report&&<button className="btn-outline" onClick={()=>window.print()}><Icon name="printer" size={14}/> Print</button>}
                    </div>
                  </div>
                  {report?(
                    <>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:12}}>
                        {[
                          {l:"Period",      v:`${MONTHS[report.month-1]} ${report.year}`,bg:"linear-gradient(135deg,#1459A0,#2D7DD2)",icon: <Icon name="calendar" size={20}/>},
                          {l:"Total Income",v:`KES ${(report.totalIncome||0).toLocaleString()}`,bg:"linear-gradient(135deg,#1A4A1A,#2E8B2E)",icon: <Icon name="dollarSign" size={20}/>},
                          {l:"Transactions",v:report.transactions,bg:"linear-gradient(135deg,#1A1A6B,#534AB7)",icon: <Icon name="hash" size={20}/>},
                          {l:"Average",     v:report.transactions>0?`KES ${Math.round((report.totalIncome||0)/report.transactions).toLocaleString()}`:"KES 0",bg:"linear-gradient(135deg,#0A3D2B,#0A7A4B)",icon: <Icon name="barChart" size={20}/>},
                        ].map(c=>(
                    ...
                    <div style={{textAlign:"center",padding:"44px 0",color:T.subtext}}>
                      <p style={{fontSize:36,marginBottom:10}}><Icon name="fileText" size={20}/></p>
              ...
              {page==="sms"&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    <h2 style={cTit}><Icon name="megaphone" size={20}/> Broadcast to All Tenants</h2>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10,flexWrap:"wrap",gap:8}}>
                      ...
                      <button className="btn-green" onClick={sendBroadcast} disabled={broadcasting}>
                        {broadcasting?"Sending...":<><Icon name="messageSquare" size={20}/> Send to All</>}
                      </button>
                    </div>
                  </div>
                  ...
                  {reminders.length>0&&(
                    <div style={card()}>
                      <h2 style={{...cTit,display:"flex",alignItems:"center",gap:8}}><Icon name="alertTriangle" size={20}/> Due Soon <Tag bg="#FAEEDA" color="#854F0B">{reminders.length}</Tag></h2>
                      {reminders.map((r,i)=>(
                        <div key={i} style={{background:"#FAEEDA",borderRadius:10,padding:"11px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:8}}>
                          ...
                          <button className="btn-orange" onClick={()=>{const t=tenants.find(x=>x.name===r.name);if(t)sendSMS(t._id,t.phone);}}><Icon name="messageSquare" size={20}/> Send</button>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={card()}>
                    <h2 style={{...cTit,marginBottom:12}}><Icon name="mail" size={20}/> Individual SMS</h2>
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      {tenants.map(t=>(
                        <div key={t._id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:T.statRow,borderRadius:10,flexWrap:"wrap"}}>
                          ...
                          <input className="inp" placeholder="Custom message (optional)" style={{flex:2,minWidth:140}}
                            value={smsMsg[t._id]||''} onChange={e=>setSmsMsg(p=>({...p,[t._id]:e.target.value}))}/>
                          <button className="btn-orange" onClick={()=>sendSMS(t._id,t.phone)} disabled={smsBusy[t._id]}>
                            {smsBusy[t._id]?"...":<Icon name="messageSquare" size={20}/>}
                          </button>
                        </div>
                      ))}
                      {!tenants.length&&<Empty T={T} icon={<Icon name="user" size={20}/>} text="No tenants yet"/>}
                    </div>
                  </div>
                </div>
              )}
              ...
              {page==="settings"&&(
                <div style={{animation:"fadeUp 0.3s ease"}}>
                  <div style={card()}>
                    <h2 style={cTit}><Icon name="shield" size={20}/> Two-Factor Authentication</h2>
                    <TwoFactorSettings API={API} auth={auth} T={T} show={show} Lbl={Lbl}/>
                  </div>
                  <div style={card()}>
                    <h2 style={cTit}><Icon name="bell" size={20}/> Login Alert Settings</h2>
